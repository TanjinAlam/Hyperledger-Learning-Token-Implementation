...
contract SkillToken is  AccessControlEnumerable, ERC1155, Ownable{
    ...
    event SkillTokenMinted(address indexed holderAddress,uint256 indexed tokenId ,uint256 indexed courseId, uint256 amount);
    ...
    function mintSkillToken(
        ...
        emit SkillTokenMinted(_learnersAddress[0], _course._programTokenId, _courseId, amount);
        _course._programTokenId++;
    }
    ...
    function batchMintSkillToken(
        ...
        for(uint256 i = 0; i< _learnersAddress.length; i++ ){
            ...
            _course.tokenMetadataArray.push(newTokenMetadata);
            emit SkillTokenMinted(_learnersAddress[i], _course._programTokenId, _courseId, amount);
            _course._programTokenId++;
        }
    }
    ...
    function getCourseDetails (uint256 _courseId) public view returns (uint256 _programLearnerIdCounts) {
        return  courses[_courseId]._programLearnerIdCount;
    }
